...
func (h *Host) String() string {
	if h.MAC != "" {
		return fmt.Sprintf("%s (%s)", h.Address, h.MAC)
	}
	return h.Address
}
...
// Add host with MAC address
func AddHostWithMAC(db *gorm.DB, address string, status string, step string, mac string) *Host {
	lock.Lock()
	defer lock.Unlock()
